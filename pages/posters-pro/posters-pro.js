...
Page({
    ...
    selectPoster:function(){
        for(var i=0;i<this.data.posterList.length;i++){
            if(this.data.posterList[i].selected==true){
                wx.setStorageSync("attachmentType",1);
                wx.setStorageSync("attachmentID",this.data.posterList[i].ID);
                wx.setStorageSync("posterID",this.data.posterList[i].ID);
                wx.setStorageSync("picPosterImage",this.data.posterList[i].MoveImg);
                wx.setStorageSync("fileName",this.data.posterList[i].AttachmentName);
                wx.setStorageSync("filePath",this.data.posterList[i].AttachmentUrl);
            }
        }
        wx.navigateBack({
            delta:1
    ...
    searchPoster:function(page){
        ...
        util.httpRequest(server.getPicGalleryBindProductPagingListByOrgId,params,'post').then(res=>{
